...
public class ClientCnxn {
    ...
    * */
    ...
    public ClientCnxn(String chrootPath, HostProvider hostProvider, int sessionTimeout, ZooKeeper zooKeeper,
        ...
        /*
        首先就是保存一些对象参数，此时的sessionId和sessionPasswd都还没有。然后就是两个timeout参数：
        connectTimeout和readTimeout。在ClientCnxn的发送和接收数据的线程中，会不断的检测连接超时和读取超时，一旦出现超时，就认为服务不稳定，
        需要更换服务器，就会从HostProvider中获取下一个服务器地址进行连接。
    ...
    class EventThread extends Thread {
        ...
        /*
        外提供了三个方法来添加不同类型的事件，如SendThread线程就会调用这三个方法来添加事件。其中对于事件通知，
        会首先根据ZKWatchManager watchManager来获取关心该事件的所有Watcher，然后触发他们。
