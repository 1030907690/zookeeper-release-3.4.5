...
public class QuorumCnxManager {
     ...
     */
    // 发送器集合。每个SendWorker消息发送器，都对应一台远程zookeeper服务器，负责消息的发送。同样，在senderWorkerMap中，也按照SID进行了分组
    final ConcurrentHashMap<Long, SendWorker> senderWorkerMap;
    //消息发送集合 用于保存哪些待发送的消息。queueSendMap是一个Map,按照sid进行分组，分别为集群中的每台机器分配了一个单独队列，从而保证各台机器之间的消息发送互不影响
    ...
    public boolean initiateConnection(Socket sock, Long sid) {
        ...
        //如果提议失败，则断开新连接
        if (sid > self.getId()) {
            LOG.info("具有较小的服务器标识符，因此删除" + "连接:(" + sid + "," + self.getId()+")");
    ...
    public void toSend(Long sid, ByteBuffer b) {
         ...
         */
        //如果sid即为本机节点，则转成Notification直接放入到recvqueue中,因为本机不再需要走网络IO
        if (self.getId() == sid) {
            b.position(0);
            ...
            //否则放入到queueSendMap中，key是要发送给哪个服务器节点的sid，ByteBuffer即为ToSend的内容，
            // queueSendMap维护的着当前节点要发送的网络数据信息，由于发送到同一个sid服务器可能存在多条数据，所以queueSendMap的value是一个queue类型；
             /*
            ...
            while ((!shutdown) && (numRetries < 3)) {
                try {
                    ...
                    while (!shutdown) {
                        ...
                        setSockOpts(client);
                        LOG.info("接收到连接请求 " + client.getRemoteSocketAddress());
