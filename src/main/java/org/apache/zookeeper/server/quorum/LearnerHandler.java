...
public class LearnerHandler extends Thread {
    ...
    public void run() {
        try {            
            ...
            /*
            总结：newEpoch计算逻辑比较简单的，大致流程如下：Learner服务器连接到Leader服务器时，会向Leader发送FOLLOWERINFO或OBSERVERINFO数据包，
            该数据包主要包含Learner节点的sid、protocolVersion、type(区分Follower还是Observer)和epoch，这里的epoch可以认为是上一代Leader的epoch值。
            Leader会收集集群中一半以上的服务器epoch信息后，选出一个最大值然后加1就是newEpoch值。
            Leader线程进入阻塞直到过半节点参与计算并产生新的newEpoch，该阶段任务才算完成，Leader线程才会被唤醒继续执行下一个阶段。为避免一直阻塞，会被设置一个超时时间，
            如果超时后epoch仍未计算结束则抛出InterruptedException，当前服务器会退出Leader角色，重新进入Leader选举状态。

            过半原则是ZAB协议中的一个核心思想，后面分析的每个阶段任务基本都是只要保证过半节点参与完成即可以代表该任务完成。
            Zookeeper集群正常工作的前提是过半服务器处于正常运行状态，因此，只要保证过半服务器参与事务变更就能保证正常集群中至少有一台服务器数据是正常的，
            又由于事务ID(zxid)递增性保证了事务的执行顺序，通过数据同步很容易实现集群中数据一致性。
            * */
            ...
            if (this.getVersion() < 0x10000) {
                ...
                /**
                 * 第三步、Learner服务器在收到带有newEpoch的LEADERINFO数据包后，会回复一个ACKEPOCH数据包,ACKEPOCH数据包同时会把Learner服务器的lastZxid带过来，
                 * 接下来Leader和Learner数据同步采用哪种方式就是根据这个lastZxid进行匹配判断确定
                 */
